...
class OrderLogic extends RelationModel
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('order')->field('tp_order.*,tp138_user_id,wh_id')->join('left join tp_users on tp_order.user_id = tp_users.user_id')->where($condition)->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    /*
     * 获取订单商品详情
