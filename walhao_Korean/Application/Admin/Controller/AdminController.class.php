...
//    }
	...
	public function index(){
		...
		$keywords = I('keywords');
		if(empty($keywords)){
			$res = D('admin')->join('left join tp_admin_attr ON tp_admin.admin_id = tp_admin_attr.admin_id')->field('tp_admin.*,tp_admin_attr.name,department,telephone,address')->select();
			// var_dump($res);
			// exit;
		}else{
			$res = D()->query("SELECT __PREFIX__admin.*,__PREFIX__admin_attr.name,department,telephone,address FROM __PREFIX__admin left join __PREFIX__admin_attr ON __PREFIX__admin.admin_id = __PREFIX__admin_attr.admin_id  where user_name like '%$keywords%' order by admin_id");
		}
		$role = D('admin_role')->getField('role_id,role_name');
		if($res && $role){
			foreach ($res as $val){
	...
	public function admin_info(){
		$admin_id = I('get.admin_id',0);
		if($admin_id){
			$info = D('admin')->join('left join tp_admin_attr ON tp_admin.admin_id = tp_admin_attr.admin_id')->where("tp_admin.admin_id=$admin_id")->field('tp_admin.*,tp_admin_attr.name,department,telephone,address')->find();
			// var_dump($info);
			// exit;
	...
	public function adminHandle(){
		...
		if($data['act'] == 'edit'){
			$r=D('admin')->where('admin_id='.$data['admin_id'])->save($data);
			$r1= D('admin_attr')->where('admin_id =' .$data['admin_id'])->find();
			if($r1){
				$r2=D('admin_attr')->where('admin_id='.$data['admin_id'])->save($data);
			}else{
				$r2=D('admin_attr')->add($data);
			}
		}
