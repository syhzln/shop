 ...
 *   
     ...
     * @DateTime  2017-02-08T13:10:15+0800
     * 修改会员检索调件,并做表优化
     */
...
class UserController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        I('tp138_user_id') ? $condition['tp138_user_id'] = I('tp138_user_id') : false;
        I('wh_id') ? $condition['wh_id'] = I('wh_id') : false;
        ...
        $user_id_arr = get_arr_column($userList, 'tp138_user_id');
...
//            $sql = "select first_leader,count(1) as count  from __PREFIX__users where ";
//            foreach ($user_id_arr as $v){
//                $sql.= "first_leader = $v or ";
//            }
//            $sql = rtrim($sql,' or ')." group by first_leader";
//            $first_leader = M('users')->query($sql);
...
//        $this->assign('third_leader',$third_leader);
    ...
    public function structure(){
        $wh_id = I('get.wh_id');//1812821982
        $user = D('users');
        $first = $user->join('tp_user_level on tp_users.level = tp_user_level.level_id')->field('nickname,level_name')->where(array('wh_id'=>$wh_id))->select();
        foreach ( $first as $k=>$v){
            $father = $v['nickname'].'('.$v['level_name'].')';
        }
        $data['name'] = $father;

        $count =  $user->where(array('first_leader'=>$wh_id))->count();
        if ($count>=1){
            $child=D('users')->join('tp_user_level on tp_users.level = tp_user_level.level_id')->field('nickname,level_name,wh_id')->where(array('first_leader'=>$wh_id))->select();
            foreach ($child  as $k =>$v){
                $data['children'][$k]['name'] = $v['nickname'].'('.$v['level_name'].')';
                $res = D('users')->where(array('first_leader'=>$v['wh_id']))->count();
                if ($res){
                    $result = $this->get_child($res,$v['wh_id']);
                }
                $data['children'][$k]['children'] =$result;
        ...
        file_put_contents('./Public/data.json', $json_string);
        $this->display();
     ...
     * 获得下一级结构
     */
    function get_child($res,$wh_id=0)
    {
        $data =array() ;
        if ($res){
            $child2=D('users')->join('tp_user_level on tp_users.level = tp_user_level.level_id')->field('nickname,level_name,wh_id')->where(array('first_leader'=>$wh_id))->select();
            foreach ($child2  as $k =>$v){
                $data[$k]['name']  = $v['nickname'].'('.$v['level_name'].')';
                $res = D('users')->where(array('first_leader'=>$v['wh_id']))->count();
                if ($res){
                    $data[$k]['children'] = $this->get_child($res,$v['wh_id']);
                }
            }
        }
    ...
    public function detail(){
        ...
        if(IS_POST){
            ...
            if($password == '' && $password2 == ''){
                ...
                $_POST['password'] = sha1('!#*'.trim("{$_POST['password']}").$user['pwd_token'].'tps');
            }
    ...
    */
     ...
     */
    public function delete(){
        $row = M()->execute('update  __PREFIX__users set is_delete = 1 where user_id = '.I('get.id'));
        if($row){
            $this->success('成功删除会员');
        }else{
            $this->error('操作失败');
    ...
    public function changePassword(){
        if(IS_POST){
            ...
            if($password!=$repassword){
                $this->error("两次输入密码不一致",U('Admin/User/changePassword'));
                exit();
            }
            $user = M('users')->where("tp138_user_id='{$username}' OR mobile='{$username}' OR email='{$username}' OR wh_id='{$username}'")->find();
            if(!$user){
                $this->error("账号不存在",U('Admin/User/changePassword'));
                exit();
            }elseif($user['is_lock'] == 1||$user['status']==3){
                $this->error("账号异常已被锁定！！！",U('Admin/User/changePassword'));
                exit();
            }elseif($user['is_delete'] ==1){
                $this->error("账号不存在!",U('Admin/User/changePassword'));
                exit();
            }else{
                $newpassword = sha1('!#*'.trim("$password").$user['pwd_token'].'tps');
                $result = M('users')->where("user_id = {$user['user_id']}")->setField('password',$newpassword);
                $this->success("操作成功",U('Admin/User/changePassword'));
                exit();
            }
        }
        $this->display();
    ...
    public function user_message(){
        ...
        $talklist = $User->where('is_talk=0')->limit($Page->firstRow.','.$Page->listRows)->select();
 ...
 //回复会员工单
    public function replyMessage(){
        // $uid=I('get.uid');
        // $uid=(int)trim($uid);
        $where['wh181_id']=I('get.whid');
        $where['message_number']=I('get.message_number');
        ...
        $talks=M('user_message')->where($where)->select();
             ...
             if(IS_POST){
                ...
                $data['k_message']=I('post.texts');
                $data['is_talk']=1;
                $res=M('user_message')->where($whe)->save($data);
    ...
    public function message_delete(){
        $uid = I('get.id/d');
        $data['is_talk']=1;
        $a=M('user_message_list')->where("id=".$uid)->save($data);
        
        $this->success("操作成功");
    }
//已完结工单
    public function finished_message(){
        $talklist=M('user_message_list')->where('is_talk=1')->select();
        $this->assign('talklist',$talklist);
       // $this->assign('page',$show);
        return $this->display();
    }
