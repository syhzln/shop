 ...
 * @DateTime  2017-09-25T09:48:31+0800
 * @return    [type]                   [description]
 */
    public function notice(){
        $notice_id = I('notice_id');
        $info = M('notice')->where(['notice_id'=>$notice_id])->find();
        if($info) $info['act'] = 'edit';
        else $info['act'] ='add';
        $this->assign('info',$info);
        if(IS_POST){
            $publish_time = empty(I('publish_time'))?time():strtotime(I('publish_time'));
            $data = I('post.');
            ////$a=$data[content];
            $data[content]=htmlspecialchars_decode($data[content]);
            //var_dump($a);die;
            $data['act']=='add'&&
            M('notice')->add([title=>$data['title'],content=>$data[content],type=>$data[type],release_time=>$publish_time,admin_id=>session('admin_id')]);
            $data['act'] =='edit' &&
            M('notice')->where(['notice_id'=>$data[notice_id]])->save([title=>$data['title'],content=>$data[content],type=>$data[type],release_time=>$publish_time,admin_id=>session('admin_id')]);
        $this->success("操作成功",U('Admin/Article/notice_list'));
        }
        $this->display();
    ...
    public function getNoticeById(){
        ...
        $notice = M('notice')->where([notice_id=>$notice_id])->getField('content');
        exit(htmlspecialchars_decode($notice));
    }
    public function guizhang_list(){
        $all = M('guizhang')->select();
        $this->assign('notice',$all);
        $this->display();
    }
    ...
    public function guizhang(){
        $notice_id = I('notice_id');
        $info = M('guizhang')->where(['notice_id'=>$notice_id])->find();
        if($info) $info['act'] = 'edit';
        else $info['act'] ='add';
        $this->assign('info',$info);
        if(IS_POST){
            ...
            //var_dump($a);die;
            $data['act']=='add'&&
            M('guizhang')->add([title=>$data['title'],content=>$data[content],type=>$data[type],release_time=>$publish_time,admin_id=>session('admin_id')]);
            $data['act'] =='edit' &&
            M('guizhang')->where(['notice_id'=>$data[notice_id]])->save([title=>$data['title'],content=>$data[content],type=>$data[type],release_time=>$publish_time,admin_id=>session('admin_id')]);
        $this->success("操作成功",U('Admin/Article/guizhang_list'));
        }
        $this->display();

    }
    public function getGuizhangById(){
        $notice_id = I('notice_id');

        $notice = M('guizhang')->where([notice_id=>$notice_id])->getField('content');
        exit(htmlspecialchars_decode($notice));
    }
}