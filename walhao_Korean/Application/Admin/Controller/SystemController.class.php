...
class SystemController extends BaseController{
     ...
     */
     ...
     function ajax_get_action()
     {
     	...
     	$type = I('type',0);    	
     	if($type>0){
     		$advContrl = get_class_methods("Seller\\Controller\\".$control);
     		$baseContrl = get_class_methods('Seller\Controller\BaseController');
     	}else{
     ...
     function right_list(){
     	...
     	$type = I('type',0);
     	if($type>0){
     		$group = array('goods'=>'商品管理','order'=>'订单物流','promtion'=>'促销管理','store'=>'店铺管理',
     				'service'=>'售后服务','charts'=>'统计报表','mesaage'=>'客服消息','seller'=>'账号管理',
     				'finance'=>'财务管理','distribut'=>'分销管理');
     	}
     	$condition['type'] = $type;
     	$name = I('name');
     	if(!empty($name)){
     	...
     	$right_list = M('system_menu')->where($condition)->select();
