		...
		if($data['id']){
			...
			if($apply && M('store_apply')->where("id=".$data['id'])->save($data)){				
				if($data['apply_state'] == 1){
					...
					if($store_id){
						$user['password'] = encrypt($apply['seller_pwd']);
						$user['reg_time'] = time();
						$user['tp138_user_id'] = substr(time(),1);
						$user['store_prefix'] = 'seller'.substr(time(),1);
						$user_id = M('users')->add($user);

						$seller = array('seller_name'=>$apply['seller_name'],'user_id'=>$user_id,
