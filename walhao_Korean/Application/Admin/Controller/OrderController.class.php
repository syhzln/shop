 ...
 * @DateTime  2017-03-08T10:49:09+0800
 */
...
class OrderController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        if(I('order_sn'))
        {
           ...
           $sn = trim(I('order_sn'));
           $where['order_sn'] = $sn;
           $where['master_order_sn']=$sn;
           $where['_logic'] = 'or';
           $condition['_complex'] = $where ;
            // $condition['_string'] = "order_sn = $sn or master_order_sn = $sn" ;
        }    
        I('tp138_user_id') != '' ? $condition['tp138_user_id'] = trim(I('tp138_user_id')) : false;
        I('wh_id') != '' ? $condition['wh_id'] = trim(I('wh_id')) : false;
        ...
        $count = M('order')->join('left join tp_users on tp_order.user_id = tp_users.user_id')->where($condition)->count();
...
//        $send = M('sendorder')->where('order_sn ='.$order['order_sn'])->getField('send_time');
//        $this->assign('send',$send);
    ...
    public function rsend(){
        ...
        $res = M('order')->execute('update tp_order set is_send = 0 where order_sn = '.I('order_sn'));
        if($res){
            exit('1');
        }else{
            exit('0');
...
//        exit;
    	...
    	$strTable .= '<td style="text-align:center;font-size:12px; width:400px;">商品信息</td>';
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="100">成本价</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="100">物流信息</td>';
    	$strTable .= '</tr>';
    	...
    	foreach($orderList as $k=>$val){
    		...
    		foreach($orderGoods as $goods){
    			$strGoods .= "商品编号：".$goods['goods_sn']."商品名称：".$goods['goods_name']." <br/> 购买数量:".$goods['goods_num']."  市场价:".$goods['market_price']."  本店售价:".$goods['goods_price']."  成本价:".$goods['cost_price']."<br/>";
    			if ($goods['spec_key_name'] != '') $strGoods .= " 规格：".$goods['spec_key_name'];
    			$strGoods .= "<br />";
                $cost[]= $goods['goods_num'] *$goods['cost_price'];
    		}
    		unset($orderGoods);
    		$strTable .= '<td style="text-align:left;font-size:12px;">'.$strGoods.' </td>';
            $strTable .= '<td style="text-align:left;font-size:12px;">'.array_sum($cost).'</td>';
                $express_no = M('delivery_doc')->where('order_sn = "'.$val['order_sn'].'"')->getField('invoice_no');
            $strTable .= '<td style="text-align:left;font-size:12px;">&nbsp;'."$express_no".'</td>';
    		$strTable .= '</tr>';
    	}
    	$strTable .='</table>';
    	unset($orderList);
...
//      搜索条件
        ...
        $region = M('region')->getField('id,name');
        $tp138_id =I('tp138_user_id');
        //fzq edit
        if($tp138_id){
            $user_id = M('users')->where([tp138_user_id=>$tp138_id])->getField('user_id');
            $where .=" AND user_id =$user_id";
        }
        $sql = "select * from __PREFIX__order $where order by order_id";

        $orderList = D()->query($sql);
        $arraydata[] = ['订单号','wh_id','昵称','日期','收货人','收货地址','电话','订单总金额','商品金额','支付方式','支付状态','发货状态','商品信息','编号','数量','成本价','运单号','处理备注','店铺名称'];
        $k=1;
        foreach($orderList as $val){
            $user_info = M('users')->field('wh_id,nickname')->where("user_id = $val[user_id]")->find();
            ...
            $express_no = M('delivery_doc')->where('order_id = '.$val['order_id'])->getField('invoice_no');
            $remark = M('orderbz')->where('order_id = '.$val['order_id'])->select();
            $store_name =D('store')->where('store_id='.$val['store_id'])->getField('store_name');

            foreach($orderGoods as $goods){
                $arraydata[$k][] = "\t".$val['order_sn'];
                $arraydata[$k][] = $user_info['wh_id'];
                $arraydata[$k][] = $user_info['nickname'];
                $arraydata[$k][] = date('Y/m/d',$val['add_time']);
                $arraydata[$k][] = $val['consignee'];
                $arraydata[$k][] = $region[$val['province']].$region[$val['city']].$region[$val['district']].$region[$val['twon']].$val['address'];
                $arraydata[$k][] = $val['mobile'];
                $arraydata[$k][] = $val['order_amount'];
                $arraydata[$k][] = $goods['goods_price']*$goods['goods_num'];
                $arraydata[$k][] = $val['pay_name'];
                $arraydata[$k][] = $this->pay_status[$val['pay_status']];
                $arraydata[$k][] = $this->shipping_status[$val['shipping_status']];
                $strGoods ="品名：".$goods['goods_name']." 数量:".$goods['goods_num'];
                if ($goods['spec_key_name'] != '') $strGoods .= " 规格：".$goods['spec_key_name'];

                $cost[]= $goods['goods_num'] *$goods['cost_price'];
                $bianhao =$goods['goods_sn'];
                $chengben = $goods['goods_num'] *$goods['cost_price'];
                $shuliang = $goods['goods_num'];

                $arraydata[$k][] = $strGoods;
                $arraydata[$k][] = $bianhao;
                $arraydata[$k][] = $shuliang;
                $arraydata[$k][] = $chengben;
                unset($orderGoods);
                unset($cost);
                unset($bianhao);
                unset($chengben);
                ...
                $arraydata[$k][] = " $express_no";
                unset($express_no);


                $remarkstr = '';
                foreach($remark as $v){
                    $remarkstr .= date('Y-m-d',$v['add_time']).'-'.$v['otext'].'-'.$v['admin_id']."\n";
                }
                $arraydata[$k][] = $remarkstr;
                unset($remark);

                $arraydata[$k][] = $store_name;
                $k++;
            }
