 ...
 */       
      ...
      //  print_r($refund);
        ...
        //修改开始
        if ($order_goods['is_return']==0 ){//如果该订单之前没有退换货

            $num = $order_goods['num']==0 ?  $order_goods['goods_num'] : $order_goods['num'];//取退货数量,为0则整单退,为
            $refund = $num*$order_goods['goods_price'];
            $return_goods_price = $refund ? $refund : 0; // 这个商品的退款金额
            $order_goods['num'] = $num;
...
//                $this->error("你的未结算资金不足 ￥{$return_goods_price}");
           ...
           //     M('store')->where(" store_id = ".STORE_ID)->setDec('pending_money',$user_money); // 从商家的 未结算自己里面扣除金额                           
            ...
            if($result)
            {                
                accountLog($user_id,$return_goods_price,0,'订单退货',0,$return_goods['order_id']);
            }  else {
                 $this->error("操作失败");
            }             
        ...
        $this->assign('return_order_id',$return_goods['order_id']);//获取单个退单的order_id  
        $this->assign('goods_id',$return_goods['goods_id']); 
        $this->assign('return_goods_price',$return_goods_price);
        $this->assign('return_integral',0);
