...
class FinanceController extends BaseController {
    ...
    public function add_edit_withdrawals()
    {
        ...
        $store_money = M('store')->field('store_money')->where(array('store_id'=>STORE_ID))->find();
        $withdrawals['store_money'] = $store_money['store_money'];

         $arr = array(
        '40'=>array('account_bank'=>'18058385558','account_name'=>'王伟芳','bank_name'=>'支付宝'),
        '21'=>array('account_bank'=>'15088798899','account_name'=>'金建明','bank_name'=>'支付宝'),
        '98'=>array('account_bank'=>'zhaowenj@gmail.com','account_name'=>'赵文捷','bank_name'=>'支付宝'),
        '1'=>array('account_bank'=>'13396810102','account_name'=>'斯红','bank_name'=>'支付宝'),
        '159'=>array('account_bank'=>'18668418885','account_name'=>'王宁','bank_name'=>'支付宝')
        );

        $payment_info = $arr[STORE_ID];
        $this->assign('payment_info',$payment_info);

        // $payment_info = M('store_payment_info')->where(array('store_id'=>STORE_ID))->find();
        // $this->assign('payment_info',$payment_info);
         ...
         if(IS_POST)
         {
             ...
             }else
             {
             	...
             	$store = M('store')->where("store_id=".STORE_ID)->find();
             	if($store['store_money'] < ($withdrawal+$Model->money)){
    ...
    public  function editWithdrawals(){        
        ...
        if(IS_POST)
        {
                ...
                $model->save();                               
                $this->success("操作成功!",U('Seller/Finance/remittance'),3);
                exit;
        }                      
