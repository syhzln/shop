 ...
 * @DateTime  2017-02-10T15:37:52+0800
 * 修改报表显示
 */
...
class ReportController extends BaseController{
	...
	public function index(){
		...
		$today['today_amount'] = M('order')->where("pay_time>$now AND pay_status=1 and store_id=$this->store_id and order_status in(0,1,2,4)")->sum('order_amount');//今日销售总额
		$today['today_order'] = M('order')->where("pay_time>$now and store_id=$this->store_id and pay_status=1")->count();//今日订单数
		...
		$sql .= " where pay_time>$this->begin and pay_time<$this->end and store_id=$this->store_id AND pay_status=1 and order_status in(0,1,2,4) group by gap";
