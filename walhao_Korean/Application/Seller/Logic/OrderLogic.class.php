...
class OrderLogic extends RelationModel
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res= M('order')->join("LEFT JOIN tp_return_goods ON tp_order.order_id = tp_return_goods.order_id")->where($condition)->field('tp_order.*,tp_return_goods.type')->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    /*
     * 获取订单商品详情
...
// file_put_contents('/home/res.txt', date('Y/m/d H:i:s',time()).'---'.json_encode($res).'//////////////'."\n",FILE_APPEND);
        if($res && $res['status'] ==1){
            $user_id = $order['user_id'];
            $users = M('users')->where('user_id ='.$user_id)->getField('user_id , tp138_user_id');
            if($users){
                $nickname = $users[$user_id];
                $sender = $order['mobile']; 
// file_put_contents('/home/sen.txt', date('Y/m/d H:i:s',time()).'---'.'name:'.$nickname.'---sn:'.$order['order_sn'].'---consignee:'.$order['consignee'].'//////////////'.'<br>',FILE_APPEND);             
                $params = array('user_name'=>$nickname , 'order_sn'=>$order['order_sn'],  'consignee'=>$order['consignee']);                
                $result=sendSms("5", $sender, $params);
// file_put_contents('/home/sms.txt', date('Y/m/d H:i:s',time()).'---'. json_encode($result).'//////////////'.'<br/>',FILE_APPEND);                                            
            }
        }            
