    ...
    */
    ...
    public function withdrawals()
    {
        ...
        if(IS_POST)
        {
            ...
            {
                $this->ajaxReturn(['status'=>-4,'msg'=>"你最多可提现 你最多可提现{$this->user['user_money']}"]);
            }
            if($this->user['user_money']<$distribut_need){
                $this->ajaxReturn(['status'=>-3,'msg'=>"账户余额最少达到'.$distribut_need.'才能提现"]);
            }
            ...
            if(M('withdrawals')->add($data)){
                ...
                $this->ajaxReturn(['status'=>-2,'msg'=>"提交失败,联系客服!"]);
            }
        }

        $where = " user_id = {$this->user_id}";
        $user = M('users')->where($where)->getField('user_money');
        $this->assign('user',$user);//用户余额
        $this->display();
    }
    public function withdrawals_list(){
        $withdrawals_list = M('withdrawals')->where(array('user_id' =>$this->user_id))->select();
        $this->assign('list',$withdrawals_list);
        $this->display();
    }
    public function transfer_list(){
        $transfer_list = M('user_transfer_log')
            ...
            ->select();
        $this->assign('list',$transfer_list);
        $this->display();
    }

    public function recharge_list(){
        $recharge_list = M('recharge')->where(array('user_id'=>$this->user_id))->select();
...
//        unset($list2);
    ...
    public function transfer(){
        ...
        $shouyi = M('user_account')->where(['user_id'=>$this->user_id])->getField('user_money');
