...
//        $filter_attr  = $goodsLogic->get_filter_attr($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选属性
        ...
        if($count > 0)
        {
            $goods_list = M('goods')->join('INNER JOIN __STORE__ ON __STORE__.store_id = __GOODS__.store_id')->where("goods_id in (".  implode(',', $filter_goods_id).") and special = 0")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
    ...
    {
        $filter_goods_id = M('goods')->where("is_hot = 1 and is_on_sale=1 and goods_state = 1")->field('goods_id')->select();
        $filter_goods_id2 = get_arr_column($filter_goods_id, 'goods_id');
        $count = count($filter_goods_id);
        $page = new Page($count,24);
        if($count > 0)
        {
            $hot_goods_list = M('goods')->where("special = 0 and goods_id in (" . implode(',', $filter_goods_id2) . ")")->limit($page->firstRow . ',' . $page->listRows)->select();
            $goods_images = M('goods_images')->where("goods_id in (".  implode(',', $filter_goods_id2).")")->cache(true)->select();
        }
        $this->assign('goods_list',$hot_goods_list);
        $this->assign('goods_images',$goods_images);  // 相册图片
        $this->assign('page',$page);// 赋值分页输出
        C('TOKEN_ON',false);
        $html = $this->fetch();
    ...
    {
        $filter_goods_id = M('goods')->where("is_on_sale=1 and goods_state = 1")->field('goods_id')->select();
        $filter_goods_id2 = get_arr_column($filter_goods_id, 'goods_id');
        $count = count($filter_goods_id);
        $page = new Page($count,24);
        if($count > 0)
        {
            $hot_goods_list = M('goods')->where("special = 0 and goods_id in (" . implode(',', $filter_goods_id2) . ")")->limit($page->firstRow . ',' . $page->listRows)->order('on_time desc')->select();
            $goods_images = M('goods_images')->where("goods_id in (".  implode(',', $filter_goods_id2).")")->cache(true)->select();
        }
        $this->assign('goods_list',$hot_goods_list);
        $this->assign('goods_images',$goods_images);  // 相册图片
...
//            $cat_id_arr = getCatGrandson ($id);
        ...
        if($count > 0)
        {
            $goods_list = M('goods')->where("special = 0 and is_on_sale=1 and goods_id in (".  implode(',', $filter_goods_id).")")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
            $filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
            if($filter_goods_id2)
...
//        $filter_attr  = $goodsLogic->get_filter_attr($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选属性
        ...
        if($count > 0)
        {
            $goods_list = M('goods')->join('INNER JOIN __STORE__ ON __STORE__.store_id = __GOODS__.store_id')->where("special = 1" and "goods_id in (".  implode(',', $filter_goods_id).")")->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();
            dump($goods_list);die;
            $filter_goods_id2 = get_arr_column($goods_list, 'goods_id');
            if($filter_goods_id2)
                $goods_images = M('goods_images')->where("goods_id in (".  implode(',', $filter_goods_id2).")")->cache(true)->select();
        ...
        $this->assign('page',$page);// 赋值分页输出
        C('TOKEN_ON',false);*/
        $good_list = M('goods')->where("special",1)->select();
        $good_list_count = count($good_list);
        $page = new Page($good_list_count, 16);
        $goods_list= M('goods')->where("special = 1 and is_on_sale = 1 and goods_state = 1 ")->limit($page->firstRow.','.$page->listRows)->select();
        $this->assign('page',$page);// 赋值分页输出
        $this->assign('goods_list',$goods_list);
        $this->display();
    }
