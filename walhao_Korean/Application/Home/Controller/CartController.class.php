        ...
        if($_REQUEST['act'] == 'submit_order')
        {
            foreach($coupon_id as $key => $val){
                if(empty($coupon_id[$key]) && !empty($couponCode[$key])){
                    $coupon_id[$key] = M('CouponList')->where("`code`='$couponCode[$key]' and store_id = $key")->getField('id');
                }
            }
 ...
 * @DateTime  2017-04-13T17:21:20+0800
 */
    public function cart5(){
		C('TOKEN_ON',false); // 关闭 TOKEN_ON
		$order_id = I('order_id',0);
		$master_order_sn = I('master_order_sn',false);

		if ($order_id) $where = "order_id = $order_id";

		if ($master_order_sn) $where = "master_order_sn = '$master_order_sn'";


		$order_amount = M('order')->where($where)->sum('order_amount');

    	$order_info = M('order')->where($where)->find();

    	if(!$order_info) $this->error('Parameter error');
		...
		if(IS_POST){
			...
			if(empty($postdata['order_id']) && empty($postdata['orderNo'])) $this->error('Parameter error');
			if(empty($postdata['orderNo'])){
				$order = M('order')->Field('order_amount,order_sn')->find($postdata[order_id]);
				$postdata['orderNo'] = 'O'.$order['order_sn'];
				$postdata['orderAmount'] = $order['order_amount'];
			}else{
				$postdata['orderAmount'] = M('order')->where("master_order_sn = '$postdata[orderNo]'")->sum('order_amount');
			}

			$postdata['orderCurrency'] = 'USD';
			$postdata['paymentMethod'] = 'Credit Card';
			$postdata['merNo'] = '10951';
			$postdata['gatewayNo'] = '10951001';
			$postdata['returnUrl'] = "http://119.23.136.241/home/cart/gloabebillpay";
			$SignKey = 'Nvh2zb06';
			unset($postdata['order_id']);
			$postdata['signInfo'] = hash("sha256",$postdata['merNo'].$postdata['gatewayNo'].$postdata['orderNo'].$postdata['orderCurrency'].$postdata['orderAmount'].$postdata['returnUrl'].$SignKey);
            // $url = "https://payment.globebill.com/Interface";
            // $html = $this->createAutoFormHtml($postdata,$url);
            // dump($html);            
			foreach($postdata as $k=>$v){
				$str.=$k.'='.$v.'&';
			}
			$str = substr($str,0,-1);
			header("Location:https://payment.globebill.com/Interface?$str");
		}

		$this->assign('order',$order_info);
		$this->assign('master_order_sn', $master_order_sn); // 主订单号
		$this->assign('sum_order_amount', $order_amount); // 所有订单应付金额
		$this->assign('pay_date',date('Y-m-d', strtotime("+1 day")));
		$this->display();
	}
...
/**
 * gloabebillpay 支付回调修改订单状态
 * @Authorhtl {Ning<nk11@qq.com>}
 * @DateTime  2017-04-13T17:23:10+0800
 */
    public function gloabebillpay(){
		C('TOKEN_ON',false); // 关闭 TOKEN_ON

		$signInfo = I('post.signInfo');
    	$postdata = I('post.');

    	$master_order_sn = I('post.orderNo');

    	if(substr($master_order_sn,0,1)=='O' ) $where = "order_sn = '".substr($master_order_sn,1)."'";
    	else $where = "master_order_sn = '$master_order_sn'";

		$orderAmount = M('order')->where($where)->sum('order_amount');
		$merNo = '10951';
		$gatewayNo = '10951001';
		$SignKey = 'Nvh2zb06';
		$check = hash("sha256",$merNo.$gatewayNo.$postdata['tradeNo'].$master_order_sn.'USD'.$orderAmount.$postdata['orderStatus'].$postdata['orderInfo'].$SignKey);
        
		if(strtoupper($check)!=$signInfo||$postdata['orderStatus']!=1) $this->error($postdata['orderInfo'],U('cart4',['master_order_sn'=>$master_order_sn]),10);


		else M('order')->where($where)->save(array('pay_code'=>'gloabebill','pay_name'=>'钱宝支付','pay_time'=>time(),'pay_status'=>$postdata['orderStatus']));

		$this->success($postdata['orderInfo'],U('Home/User/order_list'),3);
 ...
 * 拼自动提交form表单
 * @Authorhtl {Ning<nk11@qq.com>}
 * @DateTime  2017-05-11T16:43:20+0800
 * @param     array 要拼的数组对象
 * @param     str form表单提交地址
 * @return    str h5页面字符串
 */
