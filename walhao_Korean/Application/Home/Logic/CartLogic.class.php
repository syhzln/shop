 ...
 */
/**
 * cartList添加is_on_sale状态
 * @Authorhtl {Ning<nk11@qq.com>}
 * @DateTime  2017-02-16T17:00:40+0800
 */
...
class CartLogic extends RelationModel
{
    ...
    function cartList($user = array() , $session_id = '', $selected = 0,$mode =0)
    {
        ...
        foreach ($cartList as $k=>$val){
            $cartList[$k]['is_on_sale'] = M('goods')->where('goods_id = '.$cartList[$k]['goods_id'])->getField('is_on_sale');
     ...
     * @param $shipping_code 物流
     * @param $invoice_title 发票抬头
     * @param array $coupon_id 优惠
     * @param $car_price 购物车分组信息
     * @param $user_note 用户留言
     * @return array mix
     * @createdate 2018/6/20 002614:22
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = array(),$car_price,$user_note)
    {
        ...
        // 循环添加订单 多少个商家添加多少个订单//修改后,商家,订单类型创建订单,同一个商家2种不同类型的商品,可创建2 个订单
        foreach($car_price['store_order_amount'] as $k => $v)
        {

            $cartListGoods = M('Cart')->where("store_id = $k and user_id = $user_id and selected = 1")->getField('goods_id',true);

            $store['special_goods'] = $arr1 = array_intersect($cartListGoods, $special_goods);
            $store['normal_goods'] = array_diff($cartListGoods, $arr1);


            foreach ($store as $ko => $vo){//开始生成订单
                if(empty($vo)) continue;//如果只卖专区,或者只买普通那就只生成一个订单否则同一个商家也是要生成2个订单的
                ...
                $where['selected'] = 1;
                $where['goods_id'] = ['in',$vo];


                $goods_price=M('cart')->where($where)->sum('goods_price*goods_num');
                $shipping_price = $ko=='normal_goods'?$car_price['store_shipping_price'][$k]:0;//'物流价格',
                $type =  $ko=='normal_goods'?1:11;
                ...
                $data = array(
                    ...
                    'goods_price'      =>$goods_price,//每个店铺的商品价格',
                    'shipping_price'   =>$shipping_price,//'物流价格',
                    'total_amount'     =>($goods_price +$shipping_price),// 订单总额 = 商品总价 + 物流费
                    'order_amount'     =>($goods_price +$shipping_price),//'应付款金额',
                    'add_time'         =>time(), // 下单时间
                    'type'             =>$type, // 订单类型
                    'store_id'         =>$k,  // 店铺id
                );

                $order_id = M("Order")->data($data)->add();//添加订单
     ...
     */
//    public function addOrder0($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = array(),$car_price,$user_note)
//    {
//        // 仿制灌水 1天只能下 50 单  // select * from `tp_order` where user_id = 1  and order_sn like '20151217%'
//        $order_count = M('Order')->where("user_id= $user_id and order_sn like '".date('Ymd')."%'")->count(); // 查找购物车商品总数量
//        if($order_count >= 50)
