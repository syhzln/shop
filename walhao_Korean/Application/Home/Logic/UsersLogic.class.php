...
class UsersLogic extends RelationModel
{
    ...
    public function login($username,$password){
        ...
        if(!$username || !$password){
            $result= array('status'=>0,'msg'=>'계좌 번호 또는 비밀번호를 입력하십시오.');
        }
        $user = M('users')->where("mobile='{$username}' OR email='{$username}' OR wh_id='{$username}'")->find();
        if(!$user){
           $result = array('status'=>-1,'msg'=>'계정이 존재하지 않습니다.');
        }elseif( sha1('!#*'.trim("$password").$user['pwd_token'].'tps')!= $user['password']){
           $result = array('status'=>-2,'msg'=>'잘못된 암호');
        }elseif($user['is_lock'] == 1||$user['status']==3){
           $result = array('status'=>-3,'msg'=>'계정 예외가 잠겼습니다！！！');
        }elseif($user['is_delete'] ==1){
            $result = array('status'=>-1,'msg'=>'계정이 존재하지 않습니다.');
        }
        else{
           $result = array('status'=>1,'msg'=>'로그인 성공','result'=>$user);
        }
        return $result;
    }
...
//        $map['password'] = encrypt($password);屏蔽原有的注册加密方式
        $map['pwd_token '] = md5(time());//添加加密所用token
        $map['password'] = sha1('!#*' . trim($password) . $map['pwd_token '] . 'tps');//新的加密方式
        $map['reg_time'] = time();
        $map['first_leader'] = cookie('first_leader'); // 推荐人id
        // 如果找到他老爸还要找他爷爷他祖父等
        if($map['first_leader'])
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.*,g.original_img,g.is_on_sale FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = ".$order_id;
...
//        $type = I('get.type',0);
    ...
    public function get_coupon($user_id,$type =0 ){
        ...
        $sql = "SELECT l.*,c.name,c.money,c.use_start_time,c.use_end_time,c.condition FROM __PREFIX__coupon_list".
            " l LEFT JOIN __PREFIX__coupon".
    ...
    public function get_goods_collect($user_id){
        ...
        $show = $page->show();
        //获取我的收藏列表
        $sql = "SELECT c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.original_img,g.is_on_sale,g.store_count FROM __PREFIX__goods_collect c ".
...
//        	}
            ...
            $sql =" select o.order_amount,o.add_time,o.order_sn,og.order_id,og.goods_id,og.goods_name,o.store_id,o.goods_price,og.goods_num from __PREFIX__order_goods as og
            left join __PREFIX__order as o on o.order_id = og.order_id  where o.user_id = $user_id and og.is_send = 1
...
//            return array('status'=>-1,'msg'=>'手机号码格式有误','result'=>'');
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
        ...
        //验证原密码
        $oldPwd =sha1('!#*'.trim("$old_password").$user['pwd_token'].'tps');
        if($is_update && ($user['password'] != '' && $oldPwd != $user['password']))
            return array('status'=>-1,'msg'=>'密码验证失败,','result'=>'');
        $newpwd = sha1('!#*'.trim("$new_password").$user['pwd_token'].'tps');
        $row = M('users')->where("user_id='{$user_id}'")->save(array('password'=>$newpwd));
        if(!$row)
    ...
    public function passwordForApp($user_id,$old_password,$new_password,$is_update=true){
        ...
        $row = M('users')->where("user_id='{$user_id}'")->save(array('password'=>$new_password));
        if(!$row){
     ...
     */
    public function check_validate_code($code, $sender, $session_id , $type ='mobile'){   	
        ...
        if($type == 'email'){
            ...
            $timeOut = $data['time'];
            $inValid = ($code!=$data['code'] || $data['sender']!=$sender);
  
        }else{
            
            if(!$isSmsEnable)return $res = array('status'=>1,'msg'=>'短信验证码功能关闭, 无需校验验证码');
