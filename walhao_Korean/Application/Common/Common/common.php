...
function getSpecial(){
    $special_goods = [1399,1165,1379,1381,1380,1167,1168,1448,1447,1335,1334,1263,1309,1629,1934,1933,
        1948,1433,1462,1461,1724,1457,1400,1458,1264,1824,1825,1890,1889,1944,1945,1958,
        1957,1999,1513,1512,1511,1510,2116,2115,2114,2131,2094,2007,2008,2009,2147,1955,1260,1259,2204,2203,2448,2450];//专区商品612
    //已下架 1339,1567,2226,2225,1948,1427,1428,1429,1430,1432,1827,1806,1805,1807,1451,1523,1524,1426,1930,
...
function get_navigation(){
    $navigation = M('navigation')->where('is_show',1)->order('sort')->select();
    return $navigation;
}
...
function update_user_level($user_id){
    ...
    $total_amount = M('order')->where("user_id=$user_id AND pay_status=1 and type=11 and order_status not in (3,5)")->sum('order_amount');
    if($level_info){
        ...
        if(isset($level) && $level>$user['level']){
            $updata['level'] = $level;
            if($user['user_title']<=0){
                $updata['user_title'] = 1;
            }
            //$updata['discount'] = $discount;
        }
        $result = M('users')->where("user_id=$user_id")->save($updata);
        if($result){
            check_user_title($user_id);
...
function check_user_title($user_id){


    $user_info = M('users')->find($user_id);
    $user_sons_info = M('users')->where('first_leader = '.$user_info['wh_id'])->getField('user_title',true);

    if(count($user_sons_info)>=3){
        rsort($user_sons_info);
        $new_arr = array_chunk($user_sons_info,3);

        $title = $new_arr[0][2]+1;

        if($title>=6) $title==6;
        if($user_info['level']>=1&&$title>$user_info['user_title']) M('users')->where("user_id=$user_id")->setField('user_title',$title);

    }
    $leader_info = M('users')->where('wh_id = '.$user_info['first_leader'])->find();

    if($leader_info&&($leader_info['user_title']<=$user_info['user_title'])) check_user_title($leader_info['user_id']);
...
function goods_thum_images($goods_id,$width,$height){
    ...
    //判断缩略图是否存在
    $path = "Public/upload/goods/thumb/$temp/$goods_id/";
...
function smsConfigValueScene($scene){
    ...
    $key = 'unknow';
    if($scene == 'sms_product'){
        //用户找回密码
        $key = $scene;
    }elseif($scene == 1){
        //用户注册
        $key = "regis_sms_enable";
    }else if($scene == 2){
        //用户找回密码
        $key = "forget_pwd_sms_enable";
    }else if($scene == 3){
        //客户下单
        $key = "order_add_sms_enable";
    }else if($scene == 4){
        //客户支付
        $key = "order_pay_sms_enable";
    }else if($scene == 5){
        //商家发货
        $key = "order_shipping_sms_enable";
    }else if($scene == 6){
        //修改手机号码
        $key = "bind_mobile_sms_enable";
    }
...
function sendSms($scene , $sender, $params){
    ...
    if($scene == 1){
        ...
        //客户下单
        $smsParam = "{\"consignee\":\"$consignee\",\"phone\":\"$sender\"}";
    }else if($scene == 4){
        //客户支付
        $order_sn = $params['order_sn'];
        $smsParam = "{\"order_sn\":\"$order_sn\"}";
    }else if($scene == 5){
        //商家发货
        $order_sn = $params['order_sn'];
        $smsParam = "{\"user_name\":\"$user_name\",\"order_sn\":\"$order_sn\",\"consignee\":\"$consignee\"}";
    }else if($scene == 6){
        //修改手机号码
        $smsParam = "{\"user_name\":\"$user_name\",\"code\":\"$code\"}";
    }
...
function update_pay_status($order_sn,$pay_status = 1)
{
    if(stripos($order_sn,'recharge') !== false){
        ...
        $sender = (!empty($store['service_phone'])) ? $store['service_phone'] : false;
        ...
        */
        foreach ($order_goods as $k=>$val){
            if($val['is_checkout']==1){
                continue;
            }//如果多个商品中的一个已经退款就跳出循环执行下一个
            $settlement = $goods_amount = $val['cost_price']*$val['goods_num']; //此商品该结算金额初始值
            ...
            if($val['rec_id'] == $rec_id || $val['is_send'] == 3){
                ...
                $order['store_settlement'] += $order_goods[$k]['cost_price']*$val['goods_num'];
