 ...
 * @DateTime  2017-03-14T10:58:52+0800
 * @param  商品主键   $goods_ids  int,array
 * @param  操作说明   $goods_state str
 * @param  原因       $reason   str
 */
function goodsact($goods_ids,$goods_state,$reason){
	// if(!is_array($goods_ids)) $goods_array[] = $goods_ids;
	...
	foreach ($goods_array as $key => $val) {
		...
		$store_msg = array(
			...
			'content' => "您的商品\"{$update_goods[goods_name]}\"  {$goods_state},原因:{$reason}",
			'addtime' => time(),
