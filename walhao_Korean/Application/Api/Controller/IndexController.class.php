...
//        $category_arr = M('goods_category')->where('id in(4,5,7)')->field('id,name')->limit(3)->cache(true,TPSHOP_CACHE_TIME)->select();
    ...
    public function test(){    
        if(I('id')==1) $this->ajaxReturn(['code'=>1,'data'=>'1321321321']);
        else $this->ajaxReturn(['code'=>2,'data'=>'http://walhao.com/Api/Index/WalhaoApp','vender'=>'4.0.2']);
    }

    public function WalhaoApp(){
        $http = new \Org\Net\Http;
        $filename = "Public/App/soubaoShopMobile-release_402_jiagu_sign.apk";
        $showname="walhao4.0.2.apk";
        $http->download($filename, $showname);
    }

    public function iosno(){
        if(I('nu')==1) $this->ajaxReturn(['code'=>1,'data'=>'4.0.0']);
        else $this->ajaxReturn(['code'=>2,'data'=>'下载地址','vender'=>'4.0.0']);
    }
