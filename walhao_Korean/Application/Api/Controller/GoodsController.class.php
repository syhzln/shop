...
class GoodsController extends BaseController {
    ...
    public function search(){
    	...
    	$goodsLogic = new \Home\Logic\GoodsLogic(); // 前台商品操作逻辑类    	     
    	$filter_goods_id = M('goods')->where("is_on_sale=1 and goods_state = 1 and goods_name like '%{$q}%'  ")->cache(true)->getField("goods_id",true);
    ...
    */
     ...
     * @DateTime  2017-01-20T11:07:10+0800
     * @return  json(string)图片绝对地址json对象
     */
    function goodsImg()
    {
        $goods_id  = I('goods_id');
        $width = I('width');
        $height = I('height');
        $img = M('goods')->field('original_img')->find($goods_id);
        $this->ajaxReturn($img);
