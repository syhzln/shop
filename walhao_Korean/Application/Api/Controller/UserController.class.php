...
/**
     ...
     * @DateTime  2017-02-10T11:07:15+0800
     * 修改退换货
     */
  /**
     * 更新获取订单列表存在is_on_sale状态
     * @Authorhtl {Ning<nk11@qq.com>}
     * @DateTime  2017-02-13T16:12:47+0800
     */
...
class UserController extends BaseController {
    ...
    public function reg(){
        ...
        if(check_mobile($username)){
           $res = $this->userLogic->check_validate_code($code, $sender , $session_id , $type);
    ...
    public function forgetPassword(){
            ...
            $res = $this->userLogic->check_validate_code($code, $mobile, $unique_id);
...
// file_put_contents('/web/www/Application/Api/Controller/res.txt',json_encode($order_list));   
...
//        $user_id = I('user_id',0);
    ...
    public function return_goods()
    {
        ...
        // file_put_contents('/home/rg.txt', 'order_id:'.$order_id.'///order_sn:'.$order_sn.'///goods_id:'.$goods_id.'///uid'.$this->user_id.'////type:'.$type.'///reason'.$reason."\n");
        ...
        if($c['order_status']==1){
//            $this->error('订单正在处理中,请联系商家退货','',5);
            exit(json_encode(array('status'=>-4,'msg'=>'订单正在处理中,请联系商家退货!')));
        }
...
//            $this->error('已经超过' . $confirm_time_config . "天内退货时间");
            exit(json_encode(array('status'=>-5,'msg'=>'已经超过' . $confirm_time_config . "天内退货时间")));
        }      
    ...
    Public function kd100cx(){
        $order_id = I('order_id');
        $invoice_no = M('delivery_doc')->where("order_id = $order_id")->getField('invoice_no');
        $this->ajaxReturn(kdcx($invoice_no));
