 ...
 */ 
  ...
  /**
     ...
     * @DateTime  2017-02-13T14:59:53+0800
     */
  /**
   * 增加物流获取过滤条件
   * @Authorhtl {Ning<nk11@qq.com>}
   * @DateTime  2017-02-14T15:15:43+0800
   */
...
class CartController extends BaseController {
    ...
    {                    
        ...
        // file_put_contents('/home/ct.txt',$_POST);
...
//        $user_id = I("user_id",0); // 用户id
        ...
        // file_put_contents('/home/cl.txt',json_encode($cartList));  
        if($cart_form_data)
        {
            // 修改购物车数量 和勾选状态
        ...
        {   
            ...
            {   
                $sale=M('goods')->where('goods_id = '.$v2['goods_id'])->getField('is_on_sale');
                // file_put_contents('/web/www/Application/Api/Controller/re.txt',$sale);
                $v2['is_on_sale'] = $sale;
                if($v2['store_id'] == $k)
                    $store['cartList'][] = $v2;
            }            
      ...
      //  $result['result']['total_price'] = $result['total_price'];        
        // file_put_contents('/web/www/Application/Api/Controller/res.txt',json_encode($v2));
    ...
    public function cart2()
    {
        ...
        $shippingList = M('shipping_area')->where("is_close = 1 and store_id in (".implode(',', $store_id_arr).")")->group("store_id,shipping_code")->getField('shipping_area_id,shipping_code,store_id');// 物流公司        
