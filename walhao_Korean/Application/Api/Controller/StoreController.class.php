...
class StoreController extends BaseController {
    ...
    public function index(){
        ...
        //新品
        $new_goods = M('goods')->field('goods_content',true)->where(array('store_id'=>$store_id,'is_new'=>1,'is_on_sale'=>1,'goods_state'=>1))->order('goods_id desc')->limit(10)->select();
        //推荐商品
        $recomend_goods = M('goods')->field('goods_content',true)->where(array('store_id'=>$store_id,'is_recommend'=>1,'is_on_sale'=>1,'goods_state'=>1))->order('goods_id desc')->limit(10)->select();  
        //热卖商品
        $hot_goods = M('goods')->field('goods_content',true)->where(array('store_id'=>$store_id,'is_hot'=>1,'is_on_sale'=>1,'goods_state'=>1))->order('goods_id desc')->limit(10)->select();
        
        //店铺商品总数
        $storeCount =  M('goods')->where("store_id=".$store_id)->count();
    ...
    public function searchStoreGoodsClass(){
        ...
        $list = M("goods")->where("store_id = 1 and is_on_sale = 1 and goods_state= 1")->field("goods_remark,goods_content" , true)->limit(0 , 10)->select();// ->query("select *  from __PREFIX__goods $where $limit ");      
        /*
    ...
    public function storeGoods()
    {
        ...
        $store_goods_where['goods_state'] = 1;
        if ($sort == 'sales') {
            //销量排序
            $orderBy = array(
                'sales_sum' => $sore_mode,
