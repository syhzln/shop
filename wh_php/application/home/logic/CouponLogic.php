...
class CouponLogic extends Model
{
    ...
    public function getCouponMoney($user_id, $coupon_id)
    {
        ...
        $GetUseCoupon = array(
            "member_id" => 9,
            "coupon_id" => 1,
            "type" => 3, //发放类型   0面额模板  1按用户发放  2注册 3邀请 4线下发放
            "promo_amount" => 10,
            "use_endtime" => "2017.12.21"
        );
        return $GetUseCoupon;
    }
    ...
    public function getCouponMoneyByCode($couponCode, $orderMoney)
    {
       /* $couponList = M('CouponList')->where("code", $couponCode)->find(); // 获取用户的优惠券
        ...
        $GetCouponMoneyCode = array(
                "coupon_code" => "SJHFUWKSLDJA",
                "coupon_id" => 2,
                "use_starttime" => "2017.10.20",
                "use_endtime" => "2017.10.23",
                "promo_amount" => 10
            );
        return array('status'=>1,'msg'=>'','result'=>$GetCouponMoneyCode['promo_amount']);
     }
}