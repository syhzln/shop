...
class Plugin extends Base {
    ...
    public function index(){
        ...
        $page = grpcPage('temp_id',$p,10,false);

        $provider->setPagination($page);
        $provider->setProviderId(STORE_ID);
...
//        $this->assign('login',$plugin_list['login']);
        $this->assign('shipping',$seller_data);
        $this->assign('shipping1',$admin_data);
...
//            }
    ...
    public function shipping_list_edit(){
        ...
        if(IS_POST){
            ...
            $update->setProviderId(STORE_ID);
            list($res,$status) = $client->UpdateTempInfo($update)->wait();
            if(!$res->getValue()){
                exit($this->error("配置失败",U('Seller/Plugin/index',array('type'=>'shipping'))));
            }
            exit($this->success("配置成功",U('Seller/Plugin/index',array('type'=>'shipping'))));
        }
