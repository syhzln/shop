...
class GoodsLogic extends Model
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        ...
        //echo STORE_ID;exit;
        $pid  = new Psp\Item\ProviderId1();
        $pid->setProviderId(STORE_ID);
        list($res,$status) = GRPC('cat')->GetCatListFromSeller($pid)->wait();
        foreach ($res ->getItemCategoryShowInfo() as $k=>$v){
            $goods_category[$k]['id'] = $v->getId();
            $goods_category[$k]['name'] = $v->getName();
            $goods_category[$k]['mobile_name'] = $v->getMobileName();
            $goods_category[$k]['is_recommend'] = $v->getIsRecommend();
            $goods_category[$k]['is_show'] = $v->getIsShow();
            $goods_category[$k]['commission'] = $v->getCommission();
            $goods_category[$k]['cat_group'] = $v->getCatGroup();
            $goods_category[$k]['sort_order'] = $v->getSortOrder();
...
//            }
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        // 规格表
        $specEmpty = New Psp\PBEmpty();
        list($res,$status) = $client->GetSpecList($specEmpty)->wait();
        foreach ($res->getSpecLogic() as $k => $v){
            $spec[$v->getId()] = $v->getName();
        }
        //$spec = M('Spec')->getField('id,name');

        //规格项
        list($res,$status) = $client->GetSpecItemList($specEmpty)->wait();
        foreach ($res->getSpecItemLogic() as $k => $v){
            $specItem[$v->getOptIndex()]['id'] = $v->getOptIndex();
            $specItem[$v->getOptIndex()]['item'] = $v->getOptName();
            $specItem[$v->getOptIndex()]['spec_id'] = $v->getSpecId();
        }
        //$specItem = M('SpecItem')->getField('id,item,spec_id');

        //规格项相关价格库存
        $gid = new Psp\Item\ItemId();
        $gid->setGoodsId($goods_id);
        list($res,$status) = $client->GetSpecPrice($gid)->wait();
        foreach ($res->getSkuPrice() as $k => $v){
            $keySpecGoodsPrice[$v->getSkuId()]['key'] = $v->getSkuId();
            $keySpecGoodsPrice[$v->getSkuId()]['key_name'] = $v->getSkuName();
            $keySpecGoodsPrice[$v->getSkuId()]['shop_price'] = $v->getShopPrice();
            $keySpecGoodsPrice[$v->getSkuId()]['market_price'] = $v->getMarketPrice();
            $keySpecGoodsPrice[$v->getSkuId()]['provider_price'] = $v->getProviderPrice();
            $keySpecGoodsPrice[$v->getSkuId()]['provider_shop_price'] = $v->getProviderShopPrice();
            $keySpecGoodsPrice[$v->getSkuId()]['store_count'] = $v->getStoreCount();
//            $keySpecGoodsPrice[$v->getSkuId()]['sku'] = '';
        }
        //$keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,store_count,bar_code,sku');
        ...
        foreach ($spec_arr2 as $k => $v)
        {
            ...
            $keySpecGoodsPrice[$item_key][provider_price] ? false : $keySpecGoodsPrice[$item_key][provider_price] = 0.0; // 价格默认为0.0
            $keySpecGoodsPrice[$item_key][provider_shop_price] ? false : $keySpecGoodsPrice[$item_key][provider_shop_price] = 0.0; // 价格默认为0.0
            $keySpecGoodsPrice[$item_key][shop_price] ? false : $keySpecGoodsPrice[$item_key][shop_price] = 0.0; // 价格默认为0.0
            $keySpecGoodsPrice[$item_key][market_price] ? false : $keySpecGoodsPrice[$item_key][market_price] = 0.0; // 价格默认为0.0
            $keySpecGoodsPrice[$item_key][store_count] ? false : $keySpecGoodsPrice[$item_key][store_count] = 0; //库存默认为0
            $str .="<td><input name='item[$item_key][provider_price]' value='{$keySpecGoodsPrice[$item_key][provider_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
            $str .="<td><input name='item[$item_key][provider_shop_price]' value='{$keySpecGoodsPrice[$item_key][provider_shop_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
            $str .="<td><input name='item[$item_key][shop_price]' value='{$keySpecGoodsPrice[$item_key][shop_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
