...
//        $order['address2'] = $order['address2'].$order['address'];
        ...
        $Order['member_id'] = $res->getMemberId();
        $Order['state'] = $res->getState();
        $Order['type'] = $res->getType();
        $Order['platform_id'] = $res->getPlatformId();
        $Order['money'] = $res->getMoney();
        $Order['currency'] = $res->getCurrency();
        $Order['order_date'] = $res->getOrderDate()?$res->getOrderDate()->getSecondS():0;
        $Order['order_sn'] = $res->getOrderSn();
        $Order['shipping_price'] = $res->getShippingPrice();
        $Order['pay_status'] = $res->getPayStatus();
        ...
        $Order['buyer_message'] = $res->getBuyerMessage();
        $Order['pay_date'] = $res->getPayDate();
        foreach ($res->getItems() as $k=>$v) {
            $Order['item'][$k]['name'] = $v->getName();
            $Order['item'][$k]['item_id'] = $v->getItemId();
            $Order['item'][$k]['provider_id'] = $v->getProviderId();
            $Order['item'][$k]['sku_id'] = $v->getSkuId();
            $Order['item'][$k]['price'] = $v->getPrice();
            $Order['item'][$k]['cost'] = $v->getCost();
            $Order['item'][$k]['currency'] = $v->getCurrencey();
            $Order['item'][$k]['amount'] = $v->getAmount();
            $Order['item'][$k]['sku_unit'] = $v->getSkuUnit();
            $Order['item'][$k]['thumb_image_url'] = $v->getThumbImageUrl();
        }
        return $Order;
    }
...
//        /* 选择一个随机的方案 */send_http_status('310');
    ...
    public function getOrderButton($order){
         ...
         *
         */
    	$os = $order['order_status'];//订单状态  1未确认 2已确认  3已取消 4已完成 5已作废 6已评价
    	$ss = $order['shipping_status'];//发货状态   1未发货 2已发货  4已签收
    	$ps = $order['pay_status'];//支付状态   1未支付 2已支付
        $btn = array();
