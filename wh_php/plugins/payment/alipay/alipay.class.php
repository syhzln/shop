...
{    
    ...
    public function  __construct() {   
        parent::__construct();
        $paymentPlugin = require_once("application/conf/payment.php");//取出支付配置

        $paymentPlugin = convert_arr_key($paymentPlugin, 'code');
        foreach ($paymentPlugin as $key=>$val)
        {
            if($val['code'] != 'alipay')
            {
                unset($paymentPlugin[$key]);
            }
        }
        $config_value = unserialize($paymentPlugin['alipay']['config_value']); // 配置反序列化
    ...
    function response()
    {
        ...
        $verify_result = $alipayNotify->verifyNotify();
            if($verify_result) //验证成功
            {
                    ...
                    //$total_fee = $_POST['total_amount'];//交易总金额
                    // 支付宝解释: 交易成功且结束，即不可再做任何操作。
                    if(($_POST['trade_status'] == 'TRADE_FINISHED')||($_POST['trade_status'] == 'TRADE_SUCCESS')){

                        if(stripos($order_sn,'recharge') !== false){
                            update_recharge_status($order_sn,$_POST['total_fee']);//  充值/还款
                        }else{
                            update_pay_status($order_sn,$_POST['total_fee']); // 修改订单支付状态
                        }
...
//                     }
                    ...
                    echo "success"; // 告诉支付宝处理成功
                     file_put_contents('/www/wwwlogs/payment_log/alipay.log',json_encode($_POST).date('Ymd H:i:s')."\n",FILE_APPEND);
            }
            else 
            {                
                echo "fail"; //验证失败                                
    ...
    function transfer($data){
        $batch_alipay_config = include 'batchPay/alipay.config.php';
        require_once("batchPay/lib/alipay_submit.class.php");

        //付款详细数据格式为：流水号1^收款方账号1^收款账号姓名1^付款金额1^备注说明1|流水号2^收款方账号2^收款账号姓名2^付款金额2^备注说明2。
        $parameter = array(
            "service" => "batch_trans_notify",//批量转账接口名称
            "partner" => $batch_alipay_config['partner'],//合作身份者ID
            "notify_url" => SITE_URL.U('Home/Payment/notifyBack',array('pay_code'=>'alipay')),//回调通知地址
            "email"	=> $batch_alipay_config['email'],//付款账号
            "account_name"	=> $batch_alipay_config['account_name'],//付款账号名
            "pay_date"	=> date('Y-m-d H:i:s'),//支付日期
            "batch_no"	=> $data['batch_number'],//批量付款批次号
            "batch_fee"	=> $data['batch_fee'],//付款总金额
            "batch_num"	=> $data['batch_num'],//付款总笔数
            "detail_data" => trim($data['detail_data']),//付款详细数据
            "_input_charset" => $batch_alipay_config['input_charset']
        );
        //建立请求
        $alipaySubmit = new AlipaySubmit($batch_alipay_config);
        $html_text = $alipaySubmit->buildRequestForm($parameter,"get", "确认");
        echo $html_text;
    }
    ...
    function transfer_response()
    {
        ...
        $batch_alipay_config = include 'batchPay/alipay.config.php';
        require_once("batchPay/lib/alipay_notify.class.php");  // 请求返回
        //计算得出通知验证结果
        $alipayNotify = new AlipayNotify($batch_alipay_config); // 使用支付宝原生自带的类和方法 这里只是引用了一下 而已
        $verify_result = $alipayNotify->verifyNotify();
        if($verify_result){
            ...
            if($success_details){
                $sdata = array_filter(explode('|', $success_details));
                foreach ($sdata as $val){
                    $pay_arr = explode('^', $val);
                    $pay_id[] = $pay_arr[0];
                    update_withdraw_status($pay_arr[0],3,strtotime($pay_arr[7]),'success');
                }
//
            }
            if($fail_details){
                ...
                foreach ($fdata as $val){
                    $pay_arr = explode('^', $val);
                    update_withdraw_status($pay_arr[0],4,strtotime($pay_arr[7]),$pay_arr[5]);
                }
            }
            file_put_contents('/www/wwwlogs/payment_log/batchpay.log',json_encode($_POST).date('Ymd H:i:s')."\n",FILE_APPEND);
            echo "success"; //告诉支付宝处理成功
        }else{
