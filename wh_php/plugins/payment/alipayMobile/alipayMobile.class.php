...
{    
    ...
    public function  __construct() {   
        ...
        $config_value = unserialize($paymentPlugin['alipayMobile']['config_value']); // 配置反序列化
        foreach ($paymentPlugin as $key=>$val)
        {
            if($val['code'] != 'alipayMobile')
            {
    ...
    {         
            ...
            $parameter = array(
                        ...
                        "subject"       =>"量子时空手机订单", //订单名称，必填
                        "total_fee"	=> $order['order_amount'], //付款金额
                        "show_url"	=> "http://www.yunjip.com/", //收银台页面上，商品展示的超链接，必填
            ...
            if($verify_result) //验证成功
            {
                    ...
                    if($_POST['trade_status'] == 'TRADE_FINISHED') 
                    {
                        ...
                        if(stripos($order_sn,'recharge') !== false){
                            ...
                            update_pay_status($order_sn,$total_fee,array('transaction_id'=>$trade_no)); // 修改订单支付状态
                        }
