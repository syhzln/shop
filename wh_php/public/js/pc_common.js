 ...
 */
function AjaxAddCart(goods_id,num,to_catr,special)
{   if(special==0)special='';
	// var identity = getCookie('token');
	var cn = getCookie('cn');
    var cur_login= getCookie('curLogin');
	if(cur_login.length ==0){
           layer.msg('您还未登录,请先登录', {icon: 2},function(){
               window.location.href = "/index.php?m=Home&c=User&a=login";
           });
           return;
	}else{
        // 如果有商品规格 说明是商品详情页提交
        if($("#buy_goods_form").length > 0){
            $.ajax({
                ...
                success: function(data){
                    ...
                    if(to_catr == 1)  //直接购买
                    {
                        location.href = "/index.php?m=Home&c=Cart&a=cart&flag="+special;
                        ajax_cart_list();
                    }
                    else
                    {
                        ...
                        layer.open({
                            ...
                            area: ['490px', '386px'], //宽高
                            content:["/index.php?m=Home&c=Goods&a=open_add_cart&flag="+special,"no"],
...
function collect_goods(goods_id){
    ...
    if(cur_login.length == 0){
        layer.msg('亲,登录后再来收藏哦', {icon: 2},function(){
            window.location.href = "/index.php?m=Home&c=User&a=login";
        });
    }
	$.ajax({
