...
function ajaxDistrict(parent_id) {
  $.ajax({
    type: "get",
    url: "/index.php?m=Home&c=Api&a=getDistrictParentId",
    dataType: "json",
    data: {parent_id: parent_id},
    success: function (data) {
      var district_html = '';
...
function ajaxDispatching() {
  ...
  if(typeof(goods_id) != 'undefined' && region_id!= ''){
    $.ajax({
      type: "POST",
      dataType: 'json',
      data: {goods_id: goods_id, region_id: region_id,weight:weight,is_free_shipping:is_free_shipping},
      ...
      success: function (data) {
        if (data.status == 1) {
          ...
          if(data.result.length > 1){
            $('#shipping_freight').text(data.result[0].shipping_name+' {$Think.lang.Symbol}'+data.result[0].freight);
          }
...
function locationInitialize()
{
  ...
  if(province_id==null || city_id==null || district_id==null || province_name==null || city_name==null || district_name==null){
    province_id = 1001110000;
    city_id = 1001110100;
    district_id = 1001110101;
    province_name = '北京市';
    city_name = '市辖区';
    district_name = '东城区';
  }
